...
pub fn short_model_label(model: &str) -> &'static str {
    if model.contains("opus-4-6") {
        ...
        "opus4.1"
    } else if model.contains("opus-4-0") || model.contains("opus-4-2") {
        "opus4"
        ...
        "sonnet4.5"
    } else if model.contains("sonnet-4-1") {
        "sonnet4.1"
    } else if model.contains("sonnet-4-0") {
        "sonnet4"
...
pub fn build_tool_panel(
    ...
    if !skill_list.is_empty() {
        children.push(Node::SectionLabel {
            text: "skills".into(),
        });
...
pub fn build_chart_data(
    ...
    let time_span_min = if ts_max > ts_min {
        (ts_max - ts_min) as f64 / 60.0
    } else {
        60.0
    };
    ...
    for (si, session) in data.sessions.iter().enumerate() {
        ...
        for agent in &session.subagents {
            *agg_agents.entry(agent.agent_type.clone()).or_default() += 1;
        }
        ...
        let flush_bucket = |bkt: &Bucket,
                            si: usize,
                            in_cost_bars: &mut Vec<BarData>,
                            out_cost_bars: &mut Vec<BarData>,
                            in_cost_fresh_bars: &mut Vec<BarData>,
                            in_cost_cache_read_bars: &mut Vec<BarData>,
                            in_cost_cache_create_bars: &mut Vec<BarData>,
                            in_tok_bars: &mut Vec<BarData>,
                            in_tok_fresh_bars: &mut Vec<BarData>,
                            in_tok_cache_read_bars: &mut Vec<BarData>,
                            in_tok_cache_create_bars: &mut Vec<BarData>,
                            out_tok_bars: &mut Vec<BarData>,
                            energy_wh_bars: &mut Vec<BarData>,
                            water_ml_bars: &mut Vec<BarData>,
                            local_cost_bars: &mut Vec<BarData>,
                            energy_wh_max: &mut f64,
                            water_ml_max: &mut f64,
                            local_cost_max: &mut f64,
                            per_turn_in_cost_max: &mut f64,
                            per_turn_out_cost_max: &mut f64,
                            in_max: &mut f64,
                            out_max: &mut f64,
                            bw: f64| {
            if bkt.count == 0 {
            ...
            *out_max = out_max.max(bkt.out_tok.abs());
            ...
            out_cost_bars.push(BarData {
                x,
                height: -(bkt.out_cost),
                width: bw,
                color: Color::rgba(220, 160, 60, 180),
                session_idx: si,
            });
            in_cost_fresh_bars.push(BarData {
                x,
                height: bkt.fresh_cost,
                width: bw,
                color: Color::rgba(60, 120, 200, 220),
                session_idx: si,
            });
            in_cost_cache_read_bars.push(BarData {
                x,
                height: bkt.cr_cost,
                width: bw,
                color: Color::rgba(80, 180, 100, 160),
                session_idx: si,
            });
            in_cost_cache_create_bars.push(BarData {
                x,
                height: bkt.cc_cost,
                width: bw,
                color: Color::rgba(220, 160, 60, 160),
                session_idx: si,
            });
            in_tok_bars.push(BarData {
                x,
                height: bkt.in_tok,
                width: bw,
                color: Color::rgba(100, 160, 220, 180),
            ...
            in_tok_fresh_bars.push(BarData {
                x,
                height: bkt.in_tok_fresh,
                width: bw,
                color: Color::rgba(60, 120, 200, 220),
                session_idx: si,
            });
            in_tok_cache_read_bars.push(BarData {
                x,
                height: bkt.in_tok_cr,
                width: bw,
                color: Color::rgba(80, 180, 100, 160),
                session_idx: si,
            });
            in_tok_cache_create_bars.push(BarData {
                x,
                height: bkt.in_tok_cc,
                width: bw,
                color: Color::rgba(220, 160, 60, 160),
                session_idx: si,
            });
            out_tok_bars.push(BarData {
                x,
                height: -(bkt.out_tok),
                width: bw,
                color: Color::rgba(220, 160, 60, 180),
            ...
            energy_wh_bars.push(BarData {
                x,
                height: bkt.wh,
                width: bw,
                color: Color::rgba(120, 200, 80, 200),
                session_idx: si,
            });
            water_ml_bars.push(BarData {
                x,
                height: bkt.wml,
                width: bw,
                color: Color::rgba(100, 160, 220, 200),
                session_idx: si,
            });
            local_cost_bars.push(BarData {
                x,
                height: bkt.lc,
                width: bw,
                color: Color::rgba(220, 180, 80, 180),
                session_idx: si,
            });
        ...
        for ev in &session.events {
            match ev {
                Event::ApiCall {
                    ...
                    let turn_energy =
                        energy::estimate_for_model(&turn_tokens, model, api_cost, &energy_config);
                    ...
                    if downsample {
                        ...
                        if bid != cur_bucket_id {
                            ...
                            if let Some(bkt) = cur_bucket.take() {
                                flush_bucket(
                                    ...
                                    &mut in_cost_bars,
                                    &mut out_cost_bars,
                                    &mut in_cost_fresh_bars,
                                    &mut in_cost_cache_read_bars,
                                    &mut in_cost_cache_create_bars,
                                    &mut in_tok_bars,
                                    &mut in_tok_fresh_bars,
                                    &mut in_tok_cache_read_bars,
                                    &mut in_tok_cache_create_bars,
                                    &mut out_tok_bars,
                                    &mut energy_wh_bars,
                                    &mut water_ml_bars,
                                    &mut local_cost_bars,
                                    ...
                                    &mut water_ml_max,
                                    &mut local_cost_max,
                                    &mut per_turn_in_cost_max,
                                    &mut per_turn_out_cost_max,
                                    &mut in_max,
                                    &mut out_max,
                                    bucket_w,
                                );
                        ...
                        in_cost_bars.push(BarData {
                            x,
                            height: *input_cost_usd,
                            width: bar_w,
                            color: Color::rgba(100, 160, 220, 180),
                            session_idx: si,
                        });
                        let out_cost_color = if *has_thinking {
                            Color::rgba(180, 80, 200, 200)
                        } else {
                            Color::rgba(220, 160, 60, 180)
                        };
                        out_cost_bars.push(BarData {
                            x,
                            height: -(*output_cost_usd),
                            width: bar_w,
                            color: out_cost_color,
                            session_idx: si,
                        });
                        in_cost_fresh_bars.push(BarData {
                            x,
                        ...
                        in_tok_cache_read_bars.push(BarData {
                            x,
                            height: *cache_read_tokens as f64,
                            width: bar_w,
                            color: Color::rgba(80, 180, 100, 160),
                            session_idx: si,
                        });
                        in_tok_cache_create_bars.push(BarData {
                            x,
                            height: *cache_create_tokens as f64,
                            width: bar_w,
                            color: Color::rgba(220, 160, 60, 160),
                            session_idx: si,
                        });
                        let out_color = if *has_thinking {
                            Color::rgba(180, 80, 200, 200)
                        } else {
                            Color::rgba(220, 160, 60, 180)
                        };
    ...
    let sort_desc =
        |v: &mut Vec<(String, u32)>| v.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
    let mut tool_list: Vec<(String, u32)> = agg_tools.into_iter().collect();
    sort_desc(&mut tool_list);
    let mut skill_list: Vec<(String, u32)> = agg_skills.into_iter().collect();
    sort_desc(&mut skill_list);
    let mut read_list: Vec<(String, u32)> = agg_reads.into_iter().collect();
    sort_desc(&mut read_list);
    let mut agent_list: Vec<(String, u32)> = agg_agents.into_iter().collect();
    sort_desc(&mut agent_list);
    ...
    for (_, color, turns) in &session_turns {
        ...
        let out_tok_pts = sort_by_x(
            turns
                .iter()
                .map(|t| [t.x, t.total_out_tok as f64])
                .collect(),
        );
        ...
        total_tok_lines.push((
            *color,
            downsample_line(&in_tok_pts, max_line_pts),
            downsample_line(&out_tok_pts, max_line_pts),
        ));
    }
    ...
    for (_, color, turns) in &session_turns {
        ...
        if let Some(last) = turns.last() {
            total_energy_max =
                total_energy_max.max(last.cumulative_energy.facility_kwh.mid * 1000.0);
    ...
    let cost_rate_max = all_cost_events
        .iter()
        .map(|(_, c)| *c)
    ...
    ChartData {
        in_cost_bars,
        in_cost_fresh_bars,
        in_cost_cache_read_bars,
        in_cost_cache_create_bars,
        out_cost_bars,
        in_tok_bars,
        in_tok_fresh_bars,
        in_tok_cache_read_bars,
        in_tok_cache_create_bars,
        out_tok_bars,
        agent_xs,
        skill_xs,
        per_turn_in_cost_max,
        per_turn_out_cost_max,
        in_max,
        out_max,
        tool_list,
        skill_list,
